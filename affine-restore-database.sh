#!/usr/bin/env bash
# affine-restore-database.sh [backup-file-name]
#
# Replaces the AFFiNE database with one of the dumps the backups service wrote.
#
#   ./affine-restore-database.sh               list and ask
#   ./affine-restore-database.sh <file-name>   restore that one
#
...
#
# Set COMPOSE_PROJECT_NAME if the stack was started with a -p other than affine.
set -Eeuo pipefail

PROJECT="${COMPOSE_PROJECT_NAME:-affine}"
APP_SERVICE="affine"
...
DB_NAME="$(env_of AFFINE_DB_NAME)"; DB_USER="$(env_of AFFINE_DB_USER)"
...
if ! docker exec "$BKP" sh -c "(set -o pipefail) 2>/dev/null && set -o pipefail; set -eu
  ...
  echo "error: the restore failed part-way. The database may now be empty: restore another backup before using AFFiNE." >&2
  exit 1
fi
